        ...
        <v-form lazy-validation ref="form" class="mt-6">
          ...
          <v-card class="rounded-xl mt-1" elevation="3" v-if="!skeleton">
            ...
            <v-card-actions>
              ...
              <v-spacer></v-spacer>
              <v-btn color="primary" @click="save" :disabled="loadingSave">
                <p class="my-auto">simpan perubahan</p>
              </v-btn>
            </v-card-actions>
          </v-card>
        </v-form>
        ...
        if (this.selectedFile[0].size > 1000000) {
          ...
          try {
            ...
            if (
              ...
              response.data.data.attributes.data === 'Id Card Has Been Used'
            ) {
              this.hasSaved = true;
              this.status = false;
    ...
    if (this.$store.state.role === 'Pencaker') {
      try {
        ...
        if (response.data.data.attributes.length > 0) {
          ...
          if (response.data.data.attributes[0].dateOfBirth === null) {
            this.date = '';
          } else {
          ...
          this.district.name = response.data.data.attributes[0].district;
          ...
          // position
          if (response.data.data.attributes[0].desiredPosition !== null) {
            JSON.parse(response.data.data.attributes[0].desiredPosition).forEach(
              (i) => {
                this.position.push(
                  i,
                );
          ...
          // base 64
          if (response.data.data.attributes[0].image !== null) {
            ...
            this.selectedFile = [{ size: response2.data.size }];
          } else {
