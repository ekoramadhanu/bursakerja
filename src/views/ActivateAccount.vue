    ...
    async activated() {
      if (this.$refs.form.validate()) {
        try {
          ...
          if (response.data.data.attributes.data === 'PIN Not Match') {
            this.status = false;
            this.message = 'PIN kartu salah';
          } else if (
            response.data.data.attributes.data === 'Account Has ben Activate'
              || response.data.data.attributes.data === 'Data Has Been Activate'
          ) {
            this.status = false;
            this.message = 'kartu bursa sudah diaktifkan';
          } else if (response.data.data.attributes.data === 'Data Not Found') {
            this.status = false;
            this.message = 'Data kartu Tidak Ditemukan';
          } else {
            this.$router.push(`/register-account/${response.data.data.attributes.data}`);
          }
