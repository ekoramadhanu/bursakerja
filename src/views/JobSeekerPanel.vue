              ...
              <v-dialog v-model="dialogAdd" max-width="500px">
                ...
                <v-card>
                  <v-card-title class="primary">
                    <span
                      class="headline white--text text-capitalize font-family"
                    >
...
export default {
  ...
  methods: {
        ...
        axios({
          ...
          .then((response) => {
            if (response.data.data.message === 'File CSV Not Valid') {
              ...
              response.data.data.message === 'Bursa Card Is Already Exist'
            ) {
              this.hasSaved = true;
              this.status = false;
              this.message = 'nomor kartu sudah ada';
    ...
    openDialogDetail(item) {
      ...
      Promise.all(
        ...
        .then((response) => {
          ...
          this.province = response[0].data.data.jobSeeker[0].province;
          this.city = response[0].data.data.jobSeeker[0].city;
          this.district = response[0].data.data.jobSeeker[0].district;
          ...
          this.position = JSON.parse(response[0].data.data.jobSeeker[0].desiredPosition).join(', ');
          this.typeFile = response[1].data.type;
          const reader = new FileReader();
          reader.onload = (e) => {
    ...
    methodGetCardjobSeeker(page) {
      ...
      axios({
        baseURL: `${this.$store.state.domain}job-seeker/card/${page}`,
        method: 'get',
        headers: header,
      })
        .then((response) => {
