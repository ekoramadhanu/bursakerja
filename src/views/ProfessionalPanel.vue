  ...
  <div>
    <v-main>
              ...
              <v-toolbar-title>
                <div class="d-flex">
                  <p class="ma-0 hidden-xs-only">
                    <span class="font-family"> Daftar Profesional </span>
                  </p>
                </div>
              </v-toolbar-title>
              <v-spacer></v-spacer>
        ...
        <v-card>
          <v-toolbar class="primary">
            <div class="size-max mx-auto d-flex justify-end">
              ...
              <v-toolbar-title class="text-capitalize white--text my-auto ml-1">
                <span class="font-family"> Edit Data Profesional </span>
              </v-toolbar-title>
              <v-spacer></v-spacer>
            ...
            <div v-if="!loadingDialog">
              <v-form ref="form" lazy-validation>
                ...
                <v-autocomplete
                  ...
                  persistent-hint
                  :rules="positionRules"
                  :hint="`Data Yang Disimpan ${editedItemJobSeeker.position}`"
                  return-object
                  outlined
                  dense
                  single-line
      ...
      <v-dialog v-model="dialogDeShow" persistent max-width="450">
        <v-card>
          <v-card-title class="headline primary white--text text-capitalize">
            menonaktifkan iklan Profesional
          </v-card-title>
          <v-card-text>
            <div class="d-flex justify-start align-center pa-2">
              ...
              <p class="ma-0 black--text">
                Apakah anda yakin mmenonaktifkan iklan Profesional ? Jika "iya"
      ...
      <v-dialog v-model="dialogShow" persistent max-width="450">
        <v-card>
          <v-card-title class="headline primary white--text text-capitalize">
            mengaktifkan iklan Profesional
          </v-card-title>
          <v-card-text>
            <div class="d-flex justify-start align-center pa-2">
              <v-icon size="80" class="error--text mr-4">$warning</v-icon>
              <p class="ma-0 black--text">
                Apakah anda yakin mengaktifkan iklan Profesional ? Jika "iya"
                silahkan pilih tombol iya
              </p>
            </div>
          </v-card-text>
...
export default {
  ...
  methods: {
    ...
    saveAdd() {
      if (this.$refs.form.validate()) {
        ...
        axios({
          baseURL: `${this.$store.state.domain}professional/create`,
          method: 'post',
          headers: {
            'x-api-key': this.$store.state.apiKey,
          ...
          .then((response) => {
            if (
              ...
              === 'Data Professional Is Successfully Created'
            ) {
              this.hasSaved = true;
              this.status = true;
    ...
    openDialogUpdate(item) {
      ...
      Promise.all(
        [
          axios({
            baseURL: `${this.$store.state.domain}professional/${this.jobSeeker[this.editedIndex].id}`,
            method: 'get',
            headers: {
              'x-api-key': this.$store.state.apiKey,
              Authorization: `Bearer ${this.$cookies.get('token')}`,
          ...
          axios({
            baseURL: `${this.$store.state.domain}professional/stream/${this.jobSeeker[this.editedIndex].id}`,
            method: 'get',
            headers: {
              'x-api-key': this.$store.state.apiKey,
              Authorization: `Bearer ${this.$cookies.get('token')}`,
        ...
        .then((response) => {
          ...
          this.editedItemJobSeeker.position = item.position;
          this.editedItemJobSeeker.phone = response[0].data.data.professional[0].phone;
          this.editedItemJobSeeker.location = item.location;
          this.editedItemJobSeeker.school = item.school;
          this.editedItemJobSeeker.expired = response[0].data.data.informal[0].expired;
          this.editedItemJobSeeker.description = response[0].data.data.professional[0].description;
          const reader = new FileReader();
          reader.onload = (e) => {
            this.editedItemJobSeeker.image = e.target.result;
          };
    ...
    saveUpdate() {
      if (this.$refs.form.validate()) {
        ...
        axios({
          baseURL: `${this.$store.state.domain}professional/update/${
          ...
          .then((response) => {
            if (
              response.data.data.message
              === 'Data Professional Is Successfully Updated'
            ) {
              this.hasSaved = true;
              this.status = true;
              this.message = 'data berhasil disimpan';
    ...
    saveShow() {
      ...
      axios({
        baseURL: `${this.$store.state.domain}professional/show/${
          this.jobSeeker[this.editedIndex].id
        }`,
        method: 'patch',
        headers: {
        ...
        .then((response) => {
          if (
            response.data.data.message === 'Data Professional Is Successfully Show'
          ) {
            this.hasSaved = true;
            this.status = true;
    ...
    saveDeShow() {
      ...
      axios({
        baseURL: `${this.$store.state.domain}professional/de-show/${
          this.jobSeeker[this.editedIndex].id
        }`,
        method: 'patch',
        headers: {
        ...
        .then((response) => {
          if (
            response.data.data.message
            === 'Data Professional Is Successfully DeShow'
          ) {
            this.hasSaved = true;
            this.status = true;
            this.message = 'data berhasil simpan';
    ...
    methodGetCardjobSeeker(page) {
      ...
      axios({
        baseURL: `${this.$store.state.domain}professional/pagination-all/${page}`,
        method: 'get',
        headers: header,
      })
        .then((response) => {
          if (response.data.data.professional.length > 0) {
            ...
            let date = '';
            let color = '';
            response.data.data.professional.forEach((i) => {
              counter += 1;
  ...
  beforeCreate() {
    if (
      ...
      axios({
        baseURL: `${this.$store.state.domain}professional/pagination-all/1`,
        method: 'get',
        headers: {
          'x-api-key': this.$store.state.apiKey,
          Authorization: `Bearer ${this.$cookies.get('token')}`,
