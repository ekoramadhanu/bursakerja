  ...
  <div>
    <v-main>
              ...
              <v-toolbar-title>
                <div class="d-flex">
                  <p class="ma-0 font-weight-bold">
                    <span class="font-family"> Daftar Tenaga Profesional </span>
                  </p>
                </div>
              </v-toolbar-title>
              <v-spacer></v-spacer>
                  ...
                  <v-toolbar class="primary">
                    ...
                    <v-toolbar-title class="text-capitalize white--text">
                      <span class="font-family">
                        Tambah data pekerja profesional
                      </span>
                    </v-toolbar-title>
                    <v-spacer></v-spacer>
      ...
      >
        <v-card>
          <v-toolbar class="primary">
            ...
            <v-toolbar-title class="text-capitalize white--text">
              <span class="font-family"> Ubah data pekerja profesional </span>
            </v-toolbar-title>
            <v-spacer></v-spacer>
      ...
      <v-dialog v-model="dialogShow" persistent max-width="450">
        <v-card>
          <v-card-title class="headline primary white--text text-capitalize">
            <span class="font-family"> mengaktifkan iklan profesional </span>
          </v-card-title>
          <v-card-text>
            <div class="d-flex justify-start align-center pa-2">
              <v-icon size="80" class="error--text mr-4">$warning</v-icon>
...
export default {
  data: () => ({
    ...
    imageRules: [
      (v) => !!v || 'Foto Perkerja Profesional Tidak Boleh Kosong',
      (v) => !v
        || v.size < 1000000
        || 'Foto Perkerja Profesional Harus Kurang Dari 1MB',
    ],
    descriptionRules: [
    ...
    phoneRules: [
      (v) => !!v || 'Nomor Telepon Pekerja Profesional Tidak Boleh Kosong',
      (v) => /[0-9]/.test(v)
        || 'Nomor Telepon Pekerja Profesional Harus Angka (0-9)',
      (v) => /^08/.test(v)
        || 'Nomor Telepon Pekerja Profesional Harus Dimulai Dengan 08...',
    ],
    locationRules: [
      (v) => !!v || 'lokasi Pencaker Profesional Tidak Boleh Kosong',
    ],
    itemSchool: [],
    schoolRules: [
      (v) => !!v || 'Sekolah Pekerja Profesional Tidak Boleh Kosong',
    ],
  ...
  methods: {
    ...
    saveShow() {
      ...
      axios({
        ...
        .then((response) => {
          if (
            response.data.data.message
            === 'Data Professional Is Successfully Show'
          ) {
            this.hasSaved = true;
            this.status = true;
    ...
    methodGetCardjobSeeker(page) {
      ...
      if (this.search === '') {
        ...
        endpoint = `${this.$store.state.domain}professional/search-all/${this.search}/${page}`;
      }
