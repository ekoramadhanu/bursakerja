    ...
    async login() {
      if (this.$refs.form.validate()) {
        try {
          ...
          if (response.data.data.attributes.data === 'Password Not Match') {
            this.status = false;
            this.message = 'kata sandi anda salah';
          } else if (
            response.data.data.attributes.data === 'Data Account Not Activate'
          ) {
            this.status = false;
            this.message = 'akun bursa kerja dinonaktifkan';
          } else if (
            response.data.data.attributes.data === 'Data Account Not Registered'
          ) {
            this.status = false;
            ...
            if (response1.data.data.attributes[0].name === null) {
              this.$store.commit('changeNameUser', '');
              this.$store.commit('changeUploadData', true);
            } else if (response1.data.data.attributes[0].name.length > 21) {
              this.$store.commit(
                'changeNameUser',
                `${response1.data.data.attributes[0].name.substr(0, 21)}....`,
              );
