    ...
    async save() {
      if (this.$refs.form.validate()) {
        try {
          ...
          if (
            response.data.data.attributes.data === 'Data Contact Is Successfully Updated'
          ) {
            this.message = 'data berhasil diubah';
            this.status = true;
            this.icon = '$success';
          } else {
            this.message = 'data tidak bisa diubah';
            this.status = false;
            this.icon = '$warning';
          }
  ...
  async beforeCreate() {
    ...
    || this.$store.state.role === 'Informal') {
      ...
      try {
        ...
        this.whatssapp = response.data.data.attributes[0].whatsapp;
        ...
        this.idContact = response.data.data.attributes[0].id;
        this.skeleton = false;
      } catch (error) {
        // eslint-disable-next-line no-console
        console.log(error);
